...
Public Function BuildDirectoryPath(ByVal SubDirectory As String) As String
    ...
    ' Ensure root path doesn't end with backslash for consistent building
    If Right(RootPath, 1) = "\" Then
        RootPath = Left(RootPath, Len(RootPath) - 1)
    End If

    ' Build the path with proper separator
    If SubDirectory = "" Then
        BuildDirectoryPath = RootPath
    Else
        ' Ensure we always have exactly one backslash between root and subdirectory
        If Right(RootPath, 1) = "\" Then
