...
' **Purpose**: 64-bit Windows API user authentication functions
' **Original**: Interface_VBA/GetUserName64.bas
' **CLAUDE.md Compliance**: 32/64-bit API compatibility - NOT backwards compatible by design
' **Deployment**: For 64-bit Excel systems ONLY
Option Explicit

' **Purpose**: Windows API declaration for getting current user name (64-bit)
' **Dependencies**: advapi32.dll
' **32/64-bit Notes**: 64-bit ONLY - will not compile on 32-bit systems
...
' **Purpose**: Get current Windows username for 64-bit systems
' **Original**: GetUserName64.bas.Get_User_Name()
' **Parameters**: None
' **Returns**: String - Current Windows username
' **Dependencies**: Windows API (advapi32.dll)
' **Side Effects**: None
' **Errors**: Returns "Unknown" if API call fails
' **32/64-bit Notes**: 64-bit Excel ONLY - designed to NOT be backwards compatible
